...
Uses Divergence Boundary Condition instead of Neumann condition on current
"""
...
def simulation(ratio = 0.0, v0 = 0.0, eta=0.0, gamma=0.0):
    ...
    n0 = 1. + 0.01 * np.sin((rMid-rMin)*np.pi)
    J0 = v0 * (1. + 0.01 * np.cos(np.pi*(rMid - rMin) / 2)) * R2 / rMid
    ...
    for t in np.arange(0, T, k):
        ...
        u1 = np.copy(np.array([
            u[:,0] + (-u[:,1] / rMid) * k/4,
            u[:,1] + k/4 * gamma * (R2 / rMid * u[:,0]*v0 - u[:,1]) + k/4*(-u[:,1]**2/u[:,0]/rMid)
        ]).T)
        u = np.copy(np.array([
            u[:, 0] + (-u1[:, 1] / rMid) * k / 2,
            u[:, 1] + k / 2 * gamma * (R2 / rMid * u1[:, 0] * v0 - u1[:, 1]) + k / 2 * (-u1[:, 1] ** 2 / u1[:, 0] / rMid)
        ]).T)
        ...
        if N % 10000 == 0 and imageLog:
            fig, axes = plt.subplots(1, 2)
            ax1, ax2 = axes
            ax1.plot([l[-1] for l in n_list])
            ax1.set_title("n(L,t)")
            ax2.plot(rMid * np.array(n_list[-1]))
            ax2.set_title("J")
            plt.show()
        N += 1
