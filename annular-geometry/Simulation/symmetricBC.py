...
Uses symmetric Dirichlet conditions on the density
"""
...
def simulation(ratio = 0.0, v0 = 0.0, eta=0.0, gamma=0.0):
    ...
    def exactRelaxation(J0_, n_, radius):

        arg1 = (np.sqrt(gamma)*k/2*np.sqrt(gamma*radius**2+4*R2*v0))/radius
        arg2=np.sqrt(gamma*radius**2+4*R2*v0)*(2*J0_*np.sqrt(gamma)+gamma**1.5*n_*radius)/(gamma**2*n_*radius**2+4*n_*gamma*R2*v0)
        return 1/2 * (-gamma * n_ * radius + np.sqrt(gamma)*n_*np.sqrt(gamma*radius**2+4*R2*v0) * np.tanh(1/2*(arg1 + 2 * np.arctanh(arg2))))


    n0 = 1. + 0.1 * np.sin((rMid-rMin)*np.pi)
    J0 = (v0) * R2 / rMid * (1.1 + 0.1*np.cos((rMid - R1)*np.pi))
    ...
    for t in np.arange(0, T, k):
        ...
        # dt / 2
        u = np.copy(np.array([u[:,0] + (-u[:,1] / rMid) * k/2,u[:,1] + k/2 * gamma * (R2 / rMid * u[:,0]*v0 - u[:,1]) + k/2*(-u[:,1]**2/u[:,0]/rMid)]).T)

        # Now impose boundary conditions on left and right!
        #uLeft = np.array([[1., u[2,1]*(R1+3*h/2)/(R1-3*h/2)], [1., u[1,1]*(R1+h/2)/(R1-h/2)]])
        #uRight = np.array([[1., u[-1][1]*(R2-h/2)/(R2+h/2)], [1., u[-1,1]*(R2-3*h/2)/(R2+3*h/2)]])

        uLeft = np.array([[1., u[0,1]], [1., u[0, 1]]])
        uRight = np.array([[1., u[-1][1]], [1., u[-1, 1]]])
        ...
        for j in range(2, uBC.shape[0] - 2):
            ...
            U[j - 2] = u[j - 2] - k / h * (FPlus - FMinus) + dissipative #+ k* 0.1 * 1/h**2*((uBC[j-1]-2*uBC[j]+uBC[j+1]))
        ...
        if N % 1000 == 0 and imageLog:
            ...
            ax1, ax2 = axes
            ax1.plot([l[-1] for l in J_list])
            ax1.set_title("n(L,t)")
            ax2.plot(rMid * np.array(J_list[-1]))
            ax2.set_title("J")
            plt.show()
        N += 1
